...
\lettrine{F}or the definition and fundamental properties of triangulated categories, we refer to J.L. Verdier \cite{Verdier}. Our goal in this section is to explain Verdier's axioms—especially the octahedron axiom—and to provide some complements.
...
The functor \( H_0 : (X, n) \mapsto H_{-n}(X) \) (reduced homology) is a cohomological functor (\cite[p.~10]{Verdier}): \( (\text{h. stable}) \to (\text{abelian groups}). \) The "reduced chain complex" construction allows it to be lifted into an exact functor \cite[p.~4]{Verdier}: \( (\text{h. stable}) \to D(\text{abelian groups}). \)
...
and R.~MacPherson \cite{GMdualite,GMone}.
...
\smallskip
The following proposition, together with the finiteness theorems of \cite{Finitude}, guarantees that one does indeed obtain a triangulated category.
...
For varying $n$ these form a projective system, and the projective system of the $\Tor_{1}$ is essentially zero. One defines $D^{b}_{c}(X,\Zl)^{\leq0}$ (resp.\ $D^{b}_{c}(X,\Zl)^{\geq0}$) by the conditions $H^{i}K=0$ for $i>0$ (resp.\ $i<0$). To make sure that one really obtains a $t$-structure, the essential point is the construction of the operators $\tau_{\leq i}$; this is carried out in \cite[1.1.2]{WeilII}. The functor $H^{0}$ induces an equivalence of categories
\[
...
\begin{align*}
  ...
  \qquad\text{(\cite[Vol.~V, 1.4.9]{Illusie})} \eqtag{3.1.3.1}\\
  \Hom_{D\AAA}(\omega K,\omega L) &= H^{0}\omega\,\mathrm{RHom}(K,L)
  \qquad(\omega=\text{forgetting }F) \eqtag{3.1.3.2}\\
  \Hom_{DF\AAA}(K,L) &= H^{0}F^{0}\mathrm{RHom}(K,L) \qquad\text{(\cite[Vol.~V, 1.4.6]{Illusie}).}
  \eqtag{3.1.3.3}
\end{align*}
...
The uniqueness assertion follows from \rn{3.2.2}. To prove existence we shall take our inspiration from the proof of \rn{3.1.8} and from the theory of cohomological descent (SGA4 VI B, summarized in \cite[\S~5]{HodgeIII}).
...
\end{cases}
\]
One has (Euler--Poincaré formula, see \cite{Raynaud}):
\[
...
As in \cite[0.7]{WeilII}, we denote by a subscript $o$ an object over $\Fq$ (a scheme, or a sheaf on a scheme). Dropping this subscript indicates extension of scalars from $\Fq$ to $\Fbar$.

\section{Recollections from \texorpdfstring{\cite{WeilII}}{[1]}}
...
\num{\anch{5.1.13}.}  The main theorem of \cite{WeilII} (\cite[3.3.1 and 6.2.3]{WeilII}) is
...
\smallskip
The following theorem is a generalization to the relative case of \cite[6.2.13]{WeilII}.
...
\smallskip
\textbf{Proof for $i=1$.} In the case where $Y_{o}$ is a point, the proof of \cite{WeilII} consisted in taking a sufficiently general hyperplane section $H$ of $X$, viewing $\ell$ as the composite of a restriction morphism and a Gysin morphism:
